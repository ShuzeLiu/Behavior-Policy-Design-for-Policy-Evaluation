...
if wandb_switch:
    ...
    wandb_learning_rate = 0.001 * pow(2, wandb.config.learning_rate)
    ...
    wandb_global_repeat = wandb.config.global_repeat
else:
    wandb_env_size = 100
    wandb_policy_id = 1
    ...
    wandb_layer_structure = "32"
    wandb_global_repeat = 1


tag =  "|".join(["BPS", str(wandb_env_size), str(wandb_learning_rate), wandb_layer_structure, str(wandb_global_repeat)])
print("tag:", tag)
if wandb_switch:
    wandb.log({"tag": tag})
...
BPS_agent = TabularBPSMCAgent(config_OPE)
BPS_agent.run_all_episode()
...
print("len(BPS_agent.list_error):", len(BPS_agent.list_error))
...
for i in range(num_train_episode):
    ...
    OPE_error = BPS_agent.list_error[i]
    ...
    # if i == num_train_episode - 1 and wandb_switch:
        wandb.log({"normalized_OPE_error": OPE_error/truth_value,   \
            ...
            "normalized_OPE_estimate": BPS_agent.list_estimate[i]/truth_value,\
            ...
            "off_IS": BPS_agent.list_importance_sampling_ratio[i],\
            "off_IS_avg": BPS_agent.list_importance_sampling_ratio_avg[i],\
            "step_compare": i+1
            } )
